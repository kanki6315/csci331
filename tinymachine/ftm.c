...
/* Kenneth C. Louden                                */
/* Edited by Lea Wittie                             */
/* Edited by Tim Destan to allow fast testing       */ 
/*   without the interpreter environment            */
/****************************************************/
...
#define   IADDR_SIZE 2048
#define   DADDR_SIZE 20000
#define   NO_REGS 8
#define   PC_REG  7
...
typedef enum {
    ...
    opNOP,     /* RR     null operation - do nothing */
...
#ifdef MEM_LENGTH
int mem_length = DADDR_SIZE;
#endif

char * opCodeTab[]
= {"HALT","IN","OUT","ADD","SUB","MUL","DIV", "NOP", "????",
   /* RR opcodes */
   "LD","ST","????", /* RM opcodes */
...
int readInstructions (void) {
	...
	reg[regNo] = 0 ;
    // Setup 0'd data memory
    dMem[0] = DADDR_SIZE - 1 ;
    for (loc = 1 ; loc < DADDR_SIZE ; loc++)
	dMem[loc] = 0 ;
    // Setup instruction memory, full of Halts
    ...
    lineNo = 0 ;
    while (! feof(pgm)) {
        // Get codeline, line number, line length
	fgets( in_Line, LINESIZE-2, pgm  ) ;
	inCol = 0 ;
	lineNo++;
	lineLen = strlen(in_Line)-1 ;

        // make sure line ends in EOLN
	if (in_Line[lineLen]=='\n') in_Line[lineLen] = '\0' ;
...
STEPRESULT stepTM (void) {
	...
	do {
	    ...
	    fflush (stdin);
	    fflush (stdout);
	    fgets(in_Line, LINESIZE, stdin); //gets(in_Line);
...
int doCommand (char inCmd) {
    ...
    */
...
#ifndef MEM_LENGTH
	printf("   x              "\
	       "Toggle timing of total instructions executed"\
	       " ('go' only)\n");
#endif
#endif
#ifdef MEM_LENGTH
	printf("   m              "\
	       "Toggle count of data memory used"\
	       " ('go' only)\n");
#endif
	printf("   c(lear         "\
	...
	/***********************************/
	itimeflag = ! itimeflag;
	printf("Timing instructions now ");
	if ( itimeflag ) printf("on.\n"); else printf("off.\n");
	break;
#endif
#endif
	...
	/***********************************/
	imemflag = ! imemflag;
	printf("Counting data memory used now ");
	if ( imemflag ) printf("on.\n"); else printf("off.\n");
	break;
#endif
    ...
    stepResult = srOKAY;
    if ( stepcnt > 0 ) { // run some or all of the program

	// run the whole program
	if ( cmd == 'g' ) { // time_t starttime;
	    clock_t start_time;
	    stepcnt = 0;
	    start_time = clock();
	    ...
	    // Handle time flag 
	    if ( itimeflag ) {
		// clock_t is int-sized microseconds  CLOCKS_PER_SEC == 1000000
		clock_t end_time; double run_time;
		end_time = clock();
		run_time = (double)((int)end_time - (int)start_time) / CLOCKS_PER_SEC;
	    ...
	    // Handle memory flag
	    if ( imemflag )
		printf("Program used %d spaces of data memory.\n",
		       DADDR_SIZE - mem_length);
#endif

	    // Handle instruction count flag
	    if ( icountflag )
#ifdef NOP_COUNT
		printf("Number of instructions executed = %d.\n      (%d were NOPs --> %d real instrs) \n",
		       stepcnt, nop_count, stepcnt - nop_count);
#else
	        printf("Number of instructions executed = %d\n",stepcnt);
#endif
	...
	// run 'stepcnt' steps of the program
	else {
...
main( int argc, char * argv[] ) {
    ...
    */
    ...
    // Turn instruction printing mode on
    doCommand('p');
    
    // Execute the program
    doCommand('g');

    // We're done, loop was commented out, so no need to quit
